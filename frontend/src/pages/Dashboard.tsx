...
export function Dashboard() {
  ...
  >("");
  const [activeTab, setActiveTab] = useState<"selected period" | "all-time">("selected period");
  ...
  // Re-fetch when date filters or activeTab changes
  useEffect(() => {
    fetchDashboardData();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex border-b border-slate-200 -mb-2">
        {(["selected period", "all-time"] as const).map((tab) => (
          <button
            key={tab}
              ...
              <div className="space-y-5">
                {budgets.length > 0 ? (
                  ...
                  <div className="text-slate-400 text-sm text-center py-6">
                    Chưa có ngân sách hoạt động. Tạo ngân sách mới để theo dõi chi
                    tiêu!
                  </div>
                )}
              </div>
